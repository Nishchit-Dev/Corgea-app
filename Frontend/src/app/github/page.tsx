                        ...
                        <Card className="mt-6">
                            ...
                            <CardContent>
                                {githubConnected ? (
                                                ...
                                                <div key={scan.id} className="border rounded-lg p-3">
                                                    <div className="flex flex-col justify-between gap-2">
                                                        <div className="min-w-0">
                                                            <div className="text-sm font-medium truncate">{scan.repository.fullName}</div>
                                                            <div className="text-xs text-gray-500">
                                                                {scan.scanType} • {scan.targetBranch} • {new Date(scan.startedAt).toLocaleString()}
                                                            </div>
                                                        </div>
                                                        <div className="flex  items-center gap-2">
                                                            <span className={`text-xs text-center px-2 py-1 rounded flex-1 ${scan.status === 'completed' ? 'bg-green-100 text-green-700' : scan.status === 'running' ? 'bg-blue-100 text-blue-700' : scan.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>{scan.status}</span>
                                                            <Button   className='gap-2 bg-black/80 text-white cursor-pointer text-xs p-1 w-20 h-max m-0' onClick={() => (window.location.href = `/github/scan/${scan.id}`)}>
                                                                View
                                    ...
                                    <div className="text-sm text-gray-500">Connect GitHub to see your scans.</div>
                                )}
                            </CardContent>
                        </Card>
                    </div>
